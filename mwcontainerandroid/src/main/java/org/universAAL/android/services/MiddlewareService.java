...
public class MiddlewareService extends Service implements AALSpaceListener{
	...
	public void onCreate() {
		...
		UaalConfig.load(this); //Sync Preferences in UaalConfig util
		...
		//Load initial config through UaalConfig utility. Changing config requires restart the MW service to take effect.
		UaalConfig.load(getApplicationContext());
		...
		System.setProperty("org.universAAL.middleware.peer.is_coordinator", Boolean.toString(UaalConfig.isServiceCoord()));
		...
		new Thread(new Runnable() {
			public void run() {
				...
				//Load initial config through UaalConfig utility. Changing config requires restart the MW service to take effect.
				UaalConfig.load(getApplicationContext()); //Sync Preferences in UaalConfig util
				...
				if (intent != null) {
					...
					if (action != null) {
						if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
							...
							if (UaalConfig.isWifiAllowed()) {
								boolean modeGW = (UaalConfig.getRemoteMode() == AppConstants.REMOTE_MODE_WIFIOFF);
								switch (newWifi) {
	...
	private synchronized void restartConnector(boolean connect) {
		...
		if (connect) {
			...
			try {
				...
				Dictionary jGroupDCOnnector = UaalConfig.getProperties("mw.connectors.communication.jgroups.core");
				...
				mModJSLP = new SLPDiscoveryConnector(
						AndroidContext.THE_CONTEXT);
				Dictionary slpDConnectorProperties = UaalConfig.getProperties("mw.connectors.discovery.slp.core");
		...
		if (aalspaceflag) {
			...
			mModSPACEMANAGER = new AALSpaceManagerImpl(tempCtxt, Environment
					.getExternalStorageDirectory().getPath() + UaalConfig.getConfigDir() + "/mw.managers.aalspace.osgi");
			Dictionary aalSpaceManagerProps = UaalConfig.getProperties("mw.managers.aalspace.core");
	...
	private synchronized void startMiddleware(){
		try {
			...
			mModSPACEMODULE = new AALSpaceModuleImpl(
					c2);
			Dictionary aalSpaceModuleProp = UaalConfig.getProperties("mw.modules.aalspace.core");
			...
			addPercent(5);
			if(UaalConfig.isUIHandler()){ //No need for UI bus if no Handler
		...
		new Thread(new Runnable() {
			public void run() {
				...
				mModHANDLER = new AndroidHandler(AndroidContext.THE_CONTEXT,
						Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX + UaalConfig.getUAALUser());
				AndroidContainer.THE_CONTAINER.shareObject(
	...
	private synchronized void startGateway(){
		switch (UaalConfig.getRemoteType()) {
...
//			Dictionary gatewayProps = UaalConfig.getProperties("ri.gateway.communicator.core");
			try {
...
//				}
				...
				//TODO show error
//				Toast.makeText(getApplicationContext(),	R.string.uaal_warning_gplay, Toast.LENGTH_LONG).show();
			}
			break;
...
//			}
	...
	public static boolean isGWrequired(){
		return (UaalConfig.getRemoteMode() == AppConstants.REMOTE_MODE_ALWAYS
				|| (UaalConfig.getRemoteMode() == AppConstants.REMOTE_MODE_WIFIOFF
...
//		if (isWifiOn()) {
			...
			PreferenceManager.getDefaultSharedPreferences(this).edit().putString(AppConstants.Keys.MYWIFI, networkId).commit();
...
//		if (isWifiOn()) {
			...
			String home = PreferenceManager.getDefaultSharedPreferences(this)
					.getString(AppConstants.Keys.MYWIFI, AppConstants.NO_WIFI);
...
//		return AppConstants.WIFI_OFF;
	...
	private void notifyPercent(){
		...
		intent.putExtra(AppConstants.ACTION_UI_PROGRESS_X_PERCENTAGE,mPercentage);
	...
	public void aalSpaceJoined(AALSpaceDescriptor spaceDescriptor) {
		...
		if( UaalConfig.getRemoteMode() == AppConstants.REMOTE_MODE_WIFIOFF){
