...
public class RESTManager {
    ...
    public static synchronized String invoke(int method, String param, String cee, String originCall) {
        if (UaalConfig.getServerUSR() == AppConstants.Defaults.CONNUSR) {
            Log.e(TAG, "Still using default user. " +
                    "Calls to REST API are disabled until a correct user is set up and may lead to error messages. User: " + UaalConfig.getServerUSR());
            return null;
        }
        String json;
        switch (method) { //TODO Allow setting tenant id instead of reusing user name
            case REGISTER:
                ...
                json = POST_SPACES
                        .replace(REPLACE_ID, UaalConfig.getServerUSR())
                        .replace(REPLACE_PARAM, param);
                sendHTTP("POST",
                        UaalConfig.getServerURL() + "/spaces/",
                        json, false, null, false);
                json = POST_SPACES_S_SERVICE_CALLERS
                        .replace(REPLACE_ID, "default");
                sendHTTP("POST",
                        UaalConfig.getServerURL() + "/spaces/" + UaalConfig.getServerUSR() + "/service/callers",
                        json, false, null, false);
                json = POST_SPACES_S_CONTEXT_PUBLISHERS
                        .replace(REPLACE_ID, "default")//TODO Check if generic can send any event
                        .replace(REPLACE_PARAM, BODY_DEFAULT_PUBLISHER.replace(REPLACE_ID, UaalConfig.getServerUSR()));
                sendHTTP("POST",
                        UaalConfig.getServerURL() + "/spaces/" + UaalConfig.getServerUSR() + "/context/publishers",
                        json, false, null, false);
                break;
            case UNREGISTER:
                Log.d(TAG, "Sending REST API request to uAAL server: UNREGISTER");
                sendHTTP("DELETE",
                        UaalConfig.getServerURL() + "/spaces/" + UaalConfig.getServerUSR(),
                        null, false, null, false);
                break;
            case SENDC:
                Log.d(TAG, "Sending REST API request to uAAL server: SENDC");
                sendHTTP("POST",
                        UaalConfig.getServerURL() + "/spaces/" + UaalConfig.getServerUSR() + "/context/publishers/default",
            ...
            case SUBSCRIBEC:
                ...
                json = POST_SPACES_S_CONTEXT_SUBSCRIBERS
                        ...
                        .replace(REPLACE_PARAM, param);
                sendHTTP("POST",
                        UaalConfig.getServerURL() + "/spaces/" + UaalConfig.getServerUSR() + "/context/subscribers",
                        json, false, null, false);
                break;
            case CALLS:
            ...
            case PROVIDES:
                ...
                json = POST_SPACES_S_SERVICE_CALLEES
                        ...
                        .replace(REPLACE_PARAM, param);
                sendHTTP("POST",
                        UaalConfig.getServerURL() + "/spaces/" + UaalConfig.getServerUSR() + "/service/callees/",
                        json, false, null, false);
                break;
            case RESPONSES:
                Log.d(TAG, "Sending REST API request to uAAL server: RESPONSES");
                sendHTTP("POST",
                        UaalConfig.getServerURL() + "/spaces/" + UaalConfig.getServerUSR() + "/service/callees/" + cee,
    ...
    private static String sendHTTP(String method, String restURL, String body, boolean expectResponse, String originCall, boolean text) {
        ...
        try {
            ...
            String auth = "Basic " + Base64.encodeBytes((UaalConfig.getServerUSR() + ":" + UaalConfig.getServerPWD()).getBytes());
            URL url;
            if (originCall != null) {
            ...
            Log.d(TAG, String.format("----------------Sending HTTP request to server.-----------------\r\n" +
                    "URL: %s \r\n" +
                    "Mehod: %s \r\n" +
                    "Body: %s \r\n" + "---------------------------------------------------------------",
                    url,method,body));
